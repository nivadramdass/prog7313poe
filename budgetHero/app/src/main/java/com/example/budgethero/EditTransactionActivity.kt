...
class EditTransactionActivity : AppCompatActivity() {
    ...
    private var currentReceiptUrl: String? = null
    ...
    private val takePhotoLauncher = registerForActivityResult(ActivityResultContracts.TakePicturePreview()) { bitmap: Bitmap? ->
        bitmap?.let {
            imageReceipt.setImageBitmap(it)
            receiptBitmap = it
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_add_transaction) // Use same layout as Add
        ...
        db.collection("users").document(userId)
            ...
            .addOnSuccessListener { doc ->
                ...
                txn?.let {
                    ...
                    editTotal.setText(Math.abs(it.total).toString())
                    selectedTimestamp = it.timestamp ?: Timestamp.now()
                    textSelectedTimestamp.text = selectedTimestamp.toDate().toString()
                    // Set category spinner selection
                    if (categoryList.isNotEmpty()) {
                        val idx = categoryList.indexOf(it.category)
                        if (idx >= 0) spinnerCategory.setSelection(idx)
                    }
                    isIncome = (it.total > 0)
                    switchIncomeExpense.isChecked = isIncome

                    currentReceiptUrl = it.receiptImage
                    if (!currentReceiptUrl.isNullOrEmpty()) {
                        Glide.with(this).load(currentReceiptUrl).into(imageReceipt)
                    }
                }
            }
    }
    ...
    private fun saveTransaction() {
        ...
        if (receiptBitmap != null || receiptImageUri != null) {
            uploadReceiptImage { url ->
                updateTransactionInFirestore(userId, name, selectedTimestamp, total, url, category)
            }
        } else {
            updateTransactionInFirestore(userId, name, selectedTimestamp, total, currentReceiptUrl, category)
        }
    }
    ...
    private fun updateTransactionInFirestore(
        userId: String,
        name: String,
        timestamp: Timestamp,
        total: Double,
    ...
    private fun deleteTransaction() {
        ...
        db.collection("users").document(userId)
            ...
            .addOnSuccessListener {
                ...
                finish() // Go back to statements/activity
            }
            .addOnFailureListener {
